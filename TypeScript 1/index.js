...
for (let index = 0; index < localStorage.length; index++) {
    ...
    const type = robot.Type === 0 ? "Male" : "Female";
...
function createRobot(event) {
    ...
    if (robotName.value && robotType.value && color.value && (!canTalk.checked || canTalk.checked && robotPhrase.value)) {
        ...
        const robot = { Name: robotName.value, Type: type, Color: color.value, Phrase: robotPhrase.value, jump: canJump.checked, talk: canTalk.checked, blink: canBlink.checked };
        robotList.push(robot);
