...
for (let index = 0; index < localStorage.length; index++) {
  ...
  const type: string = robot.Type === 0 ? "Male" : "Female";
  const color: string = robot.Color;
  const phrase: string = robot.Phrase;
  const jump: boolean = robot.jump;
  const talk: boolean = robot.talk;
  const blink: boolean = robot.blink;
...
function createRobot(event: Event) {
  ...
  if (robotName.value && robotType.value && color.value && (!canTalk.checked || canTalk.checked && robotPhrase.value)) { 
    ...
    const robot: Robot = {Name: robotName.value, Type: type, Color: color.value, Phrase: robotPhrase.value, jump: canJump.checked, talk: canTalk.checked, blink: canBlink.checked};
    robotList.push(robot);

    localStorage.setItem(`slide-${robotList.length}`, JSON.stringify(robot));

    createRobotSection(robotName.value, robotType.value, color.value, robotPhrase.value, canJump.checked, canTalk.checked, canBlink.checked);
...
function createRobotSection(n:string, type:string, color:string, phrase:string, jump:boolean, talk:boolean, blink:boolean) {
...
function CreateMessage(sect: HTMLDivElement) {
  ...
  if (text.value) {
    ...
    const message: Message = {Name: name.textContent, Color: color.style.backgroundColor, Text: text.value, Date: dateP.textContent};
